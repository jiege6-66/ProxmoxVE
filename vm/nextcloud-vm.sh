...
function default_settings() {
  ...
  echo -e "${CREATING}${BOLD}${DGN}正在创建 a Nextcloud VM using the above default settings${CL}"
}
...
function advanced_settings() {
  ...
  if (whiptail --backtitle "Proxmox VE Helper Scripts" --title "ADVANCED SETTINGS COMPLETE" --yesno "Ready to create a Nextcloud VM?" --no-button Do-Over 10 58); then
    echo -e "${CREATING}${BOLD}${DGN}正在创建 a Nextcloud VM using the above advanced settings${CL}"
  else
    header_info

...
msg_info "正在获取 latest EMQX Enterprise version"
...
if [[ -z "$LATEST_VERSION" ]]; then
  msg_error "无法 determine latest EMQX version"
  exit 1
fi
...
msg_info "正在下载 EMQX v$LATEST_VERSION"
...
if [[ ${prompt,,} =~ ^(y|yes)$ ]]; then
  ...
  msg_ok "已禁用 EMQX MQ feature"
fi

msg_info "正在启动 EMQX service"
$STD systemctl enable -q --now emqx
msg_ok "已启用 EMQX service"

      ...
      {item.interface_port !== null
        ? (
            <div className="flex items-center justify-end">
              <h2 className="mr-2 text-end text-lg font-semibold">默认接口：</h2>
              <span className={cn(buttonVariants({ size: "sm", variant: "outline" }), "flex items-center gap-2")}>
                {item.interface_port}
                <ClipboardIcon onClick={() => handleCopy("默认接口", String(item.interface_port))} className="size-4 cursor-pointer" />
              </span>
            </div>
          )
        : null}

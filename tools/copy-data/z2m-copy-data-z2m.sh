...
while true; do
  read -p "用于将所有数据从一个 Zigbee2MQTT LXC 复制到另一个。是否继续(y/n)?" yn
  case $yn in
  [Yy]*) break ;;
  [Nn]*) exit ;;
  *) echo "请回答 yes 或 no。" ;;
...
版本: 2022.01.23" 13 50
info "Zigbee2MQTT 数据从 '$CTID_FROM' 到 '$CTID_TO'"
...
# Use to copy all data from one Zigbee2MQTT LXC to another
# run from the Proxmox Shell
# bash -c "$(curl -fsSL https://raw.githubusercontent.com/jiege6-66/ProxmoxVE/mainmain/tools/copy-data//z2m-copy-data-z2m.sh)"

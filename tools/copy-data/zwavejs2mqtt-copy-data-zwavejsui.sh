...
while true; do
  read -p "用于将所有数据从 Zwavejs2MQTT LXC 复制到 Z-wave JS UI LXC。是否继续(y/n)?" yn
  case $yn in
  [Yy]*) break ;;
  [Nn]*) exit ;;
  *) echo "请回答 yes 或 no。" ;;
...
TITLE="Zigbee2MQTT 到 Z-wave JS UI 数据复制"
...
版本: 2022.09.21" 13 50
info "Zwavejs2MQTT 数据从 '$CTID_FROM' 到 '$CTID_TO'"
...
# Use to copy all data from a Zwavejs2MQTT LXC to a Z-wave JS UI LXC
# run from the Proxmox Shell
# bash -c "$(curl -fsSL https://raw.githubusercontent.com/jiege6-66/ProxmoxVE/mainmain/tools/copy-data//zwavejs2mqtt-copy-data-zwavejsui.sh)"
